...
extract($data);
$token = uniqid();
$res = $read_one->read("id, email", "patients", "email=:email", ['email'=>$email]);
if ($res) {
    if ($update_record->update('patients', 'token=:token', 'id=:id', ["id"=>$res['id'], "token"=>$token])) {
        $mailing = MailUser::mail_user($mail, ['email'=>$email, 'token'=>$token, 'user'=>'patients']);
        if($mailing['mailsent']){
