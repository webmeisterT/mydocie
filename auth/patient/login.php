...
if ($_SERVER['REQUEST_METHOD'] === "POST") {
	...
	if (!empty($data) && is_array($data)) {
		...
		if (!empty(isset($email))) {
			$response = $read_one->read("id, password, email", "patients", "email=:email", ['email' => $email]);
			...
			$response = $read_one->read("id, password, username", "patients", "username=:username", ['username' => $username]);
