		...
		if (!empty(isset($email))) {
			...
			if ($response) {
				if (password_verify($password, $response['password'])) {
					...
					echo json_encode(["success" => true, "message" => "Successfully logged in", "data" => $read_one->read("id, first_name, last_name, username, email, phone, specialization, current_clinic, image", "doctors", "email=:email", ['email' => $email])]);					
				} else {					
			...
			if ($response) {
				if (password_verify($password, $response['password'])) {
					http_response_code(202);
					echo json_encode(["success" => true, "message" => "Successfully logged in", "data" => $read_one->read("id, first_name, last_name, username, email, phone, specialization, current_clinic, image", "doctors", "username=:username", ['username' => $username])]);					
				} else {					
					http_response_code(401);
					echo json_encode(["success" => false, "message" => "Invalid credentials", "data" => null]);
				}
